...
| POST   | /api/login    | Use the credentials sent inside the `body` to login the user. On successful login, create a new session for the user and send back a 'Logged in' message. If login fails, repond with the correct status code and the message: 'You shall not pass!' |
